  ...
  let(:tracks)  {
    [
      ...
      describe '#duration' do
        it 'calculates the duration of the album in minutes' do
          albums = []
          tracks.each do |track|
            ...
            new_album.tracks << Track.new(track[:album_id], track[:track_id], track[:title], track[:track_number], track[:duration_ms])
          end
          expect(new_album.duration).to eq("19:07")
        end
      end
